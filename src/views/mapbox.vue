...
<!--  </el-select>-->
    ...
    <div class="list">
  <el-select v-model="mvalue" placeholder="面积" @change="getmap" clearable >
    <el-option v-for="item in m" :key="item.value" :label="item.label" :value="item.value"> </el-option>
  </el-select>
  </div>
  <div class="list">
  <el-select v-model="moneyvalue" placeholder="价格" @change="getmap" clearable >
    <el-option v-for="item in money" :key="item.value" :label="item.label" :value="item.value"> </el-option>
  </el-select>
  </div>
  <div class="list">
  <el-select v-model="fitmentvalue" placeholder="装修程度" @change="getmap" clearable >
...
<script type="text/javascript">
	...
	export default {
		...
		methods: {
          ...
          this.mvalues=""
          this.mvalueb=""
        }else{	this.mvalues=this.mvalue.split("-")[0]
...
//              	'cityId=' +sessionStorage.getItem("Skycityid")+ '&startSpace=' + "" + '&startSpace=' + "" + '&endSpace=' + "" + '&startMoney=' + "" + '&endMoney=' + "" + '&fitment=' + ''
			...
			map(arr,centlngs,centlats) {console.log(33333311)
				...
				ComplexCustomOverlay.prototype.initialize = function(map) {
					...
					div.style.backgroundColor = "#1ead69";
          div.style.borderRadius = "400px";
          div.style.opacity = "0.8";
					...
					div.onmouseover = function() {
						this.style.backgroundColor = "#1ead69";
						this.style.borderColor = "#1ead69";
            ...
            // onclick='aaa(1)'http://m.amap.com href=''
            tables =tables+"<a  href='https://www.woowjia.com/#/details?id="+arr[i].housesNews[j].id+"'  class='flex flexas' style='margin-top: 10px;display:flex;border-bottom: 1px solid #f1f1f1;'><p class='flexssa' style='width:25%;line-height:50px'>"+arr[i].housesNews[j].work_name+"</p>"+"<p class='flexssa' style='width:25%;line-height: 50px;'>"+arr[i].housesNews[j].office_space+"m²</p>"+"<p class='flexssa' style='width:25%;line-height: 50px;'>"+arr[i].housesNews[j].money+"元/m²/天</p><p class='flexssa' style='width:25%;line-height: 50px;'>"+"<img style='display: block;height: 50px;width: 100%;' src="+arr[i].mansion_picture+"></p></a>"
					}
					var content ="<h4 style='margin:0 0 5px 0;padding:0.2em 0'>"+arr[i].mansion_name+"</h4>" + "<img style='height:150px;width:600px' class='imgDemo' src="+arr[i].mansion_picture+" >"
							+tables
							"</div>";
...
//				http://api.map.baidu.com/geocoder/v2/?address=北京市海淀区上地十街10号&output=json&ak=您的ak&callback=showLocation //GET请求
      ...
      getlist(){
        this.$axios.post("region/getRegionByid.action", {
      id:sessionStorage.getItem("Skycityid")
        }).then(res => {
...
//			this.mapseach("北京")
  ...
  //   this.getlist()
   ...
   function aaa (ids) {
      alert( document.querySelectorAll(".flexas").length)
   }
  }
	/*			windowAddMouseWheel();
